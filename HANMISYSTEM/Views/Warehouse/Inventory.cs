...
namespace HANMISYSTEM.Views.Warehouse
{
    public partial class Inventory : Form
    {
        ...
        private int GetTotalPage(string searchTearm,string wh)
        {
            ...
            {
                dataTable = connect.readdata("with temp as  (select pi.partno,c.partname,SUM(quantity) as quantity,COUNT(idpack) as packageqty from packinginfo pi inner join cargo c on pi.partno=c.partno where pi.idwarehouse ='"+wh+"' group by pi.partno,c.partname) select COUNT(*) as row from temp");
            }
            else
            {
                dataTable = connect.readdata("with temp as  (select pi.partno,c.partname,SUM(quantity) as quantity,COUNT(idpack) as packageqty from packinginfo pi inner join cargo c on pi.partno=c.partno where pi.partno like '%" + searchTearm + "%' and pi.idwarehouse ='" + wh + "' group by pi.partno,c.partname) select COUNT(*) as row from temp");
            }
        ...
        private void GetData(int page, int pageSize, string searchTearm,string wh)
        {
            try
            {
                if (string.IsNullOrEmpty(searchTearm))
                {
                    loaddata("with temp as  (select ROW_NUMBER() over (Order by pi.partno asc) as row, pi.partno,c.partname,SUM(quantity) as quantity,COUNT(idpack) as packageqty from packinginfo pi inner join cargo c on pi.partno=c.partno where pi.idwarehouse ='"+wh+"' group by pi.partno,c.partname) select *  from temp  where row >=" + ((page * pageSize - pageSize) + 1) + " and row <= " + (page * pageSize) + "");
                }
                else
                {
                    loaddata("with temp as  (select ROW_NUMBER() over (Order by pi.partno asc) as row, pi.partno,c.partname,SUM(quantity) as quantity,COUNT(idpack) as packageqty from packinginfo pi inner join cargo c on pi.partno=c.partno where  pi.partno like '%"+searchTearm+"%' and pi.idwarehouse ='"+wh+"' group by pi.partno,c.partname) select *  from temp  where row >=" + ((page * pageSize - pageSize) + 1) + " and row <= " + (page * pageSize) + "");
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                dt= connect.readdata("with temp as  (select ROW_NUMBER() over (Order by pi.partno asc) as row, pi.partno,c.partname,SUM(quantity) as quantity,COUNT(idpack) as packageqty from packinginfo pi inner join cargo c on pi.partno=c.partno where pi.idwarehouse ='" + wh + "' group by pi.partno,c.partname) select *  from temp");
            }
            else
            {
                dt=connect.readdata("with temp as  (select ROW_NUMBER() over (Order by pi.partno asc) as row, pi.partno,c.partname,SUM(quantity) as quantity,COUNT(idpack) as packageqty from packinginfo pi inner join cargo c on pi.partno=c.partno where  pi.partno like '%" + txtsearch.Text + "%' and pi.idwarehouse ='" + wh + "' group by pi.partno,c.partname) select *  from temp");
            }
